...
var SupabaseDatabaseAdapter = class extends DatabaseAdapter {
  ...
  async getMemoriesByRoomIds(params) {
    ...
    let query = this.client.from("memories").select("*").eq("type", params.tableName).in("roomId", params.roomIds).order("createdAt", { ascending: false });
    if (params.agentId) {
  ...
  async getActorDetails(params) {
    try {
      const response = await this.client.from("rooms").select(
        `
  ...
  async getMemories(params) {
    const query = this.client.from("memories").select("*").eq("roomId", params.roomId);
    if (params.start) {
  ...
  async getMemoryById(memoryId) {
    try {
      const { data, error } = await this.client.from("memories").select("*").eq("id", memoryId).maybeSingle();
      if (!data) {
  ...
  async getRoomsForParticipants(userIds) {
    ...
    return Array.from(new Set(data.map((row) => row.roomId)));
  }
  ...
  async addParticipant(userId, roomId) {
    const { error } = await this.client.from("participants").insert({ userId, roomId });
    if (error) {
  ...
  async createRelationship(params) {
    ...
    const { error: relationshipError } = await this.client.from("relationships").upsert({
      userA: params.userA,
      userB: params.userB,
      userId: params.userA,
      status: "FRIENDS"
