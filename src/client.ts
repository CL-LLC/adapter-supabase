import { type Account, type Actor, type Adapter, DatabaseAdapter, elizaLogger, type Goal, type GoalStatus, type IAgentRuntime, type Memory, type Participant, type RAGKnowledgeItem, type Relationship, type Room, type UUID } from "@elizaos/core";
import { createClient, type SupabaseClient } from "@supabase/supabase-js";
import fetch from "node-fetch";
import type pino from "pino";
import { v4 as uuid } from "uuid";

// Enhanced fetch implementation with retries
const customFetch = async (url: string, options: RequestInit = {}, retries = 3, delay = 1000) => {
  let lastError;
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      lastError = error;
      elizaLogger.warn(`Fetch attempt ${i + 1} failed:`, error);
  ...
  elizaLogger.error("All fetch attempts failed:", lastError);
  throw lastError;
};
...
export class SupabaseDatabaseAdapter extends DatabaseAdapter {
  ...
  constructor(url: string, key: string, logger: pino.Logger) {
    super();
    this.logger = logger;
    this.logger.info("Initializing Supabase client with URL:", url);
  ...
  async getRoom(roomId: UUID): Promise<UUID | null> {
    const { data, error } = await this.client.from("rooms").select("id").eq("id", roomId).maybeSingle();
  ...
  async getParticipantsForAccount(userId: UUID): Promise<Participant[]> {
    const { data, error } = await this.client.from("participants").select("*").eq("userId", userId);
  ...
  async getParticipantUserState(roomId: UUID, userId: UUID): Promise<"FOLLOWED" | "MUTED" | null> {
    const { data, error } = await this.client.from("participants").select("userState").eq("roomId", roomId).eq("userId", userId).maybeSingle();
  ...
  async setParticipantUserState(roomId: UUID, userId: UUID, state: "FOLLOWED" | "MUTED" | null): Promise<void> {
    const { error } = await this.client.from("participants").update({ userState: state }).eq("roomId", roomId).eq("userId", userId);
  ...
  async getParticipantsForRoom(roomId: UUID): Promise<UUID[]> {
    const { data, error } = await this.client.from("participants").select("userId").eq("roomId", roomId);
  ...
  async init() {
    try {
      ...
      const { data, error } = await this.client.from("rooms").select("id").limit(1);
      if (error) throw error;
      elizaLogger.info("Supabase connection test successful");
    } catch (error) {
      elizaLogger.error("Failed to initialize Supabase connection:", error);
      throw error;
    }
  }
  ...
  async getAccountById(userId: UUID): Promise<Account | null> {
    const { data, error } = await this.client.from("accounts").select("*").eq("id", userId);
    if (error) {
  ...
  async searchMemories(params: { tableName: string; roomId: UUID; embedding: number[]; match_threshold: number; match_count: number; unique: boolean }): Promise<Memory[]> {
    ...
    const result = await this.client.rpc("search_memories", opts);
    if (result.error) {
  ...
  async getCachedEmbeddings(opts: { query_table_name: string; query_threshold: number; query_input: string; query_field_name: string; query_field_sub_name: string; query_match_count: number }): Promise<
    {
      embedding: number[];
      levenshtein_score: number;
    }[]
  > {
    const result = await this.client.rpc("get_embedding_list", opts);
    if (result.error) {
      throw new Error(JSON.stringify(result.error));
    }
    return result.data;
  ...
  async updateGoalStatus(params: { goalId: UUID; status: GoalStatus }): Promise<void> {
    await this.client.from("goals").update({ status: params.status }).match({ id: params.goalId });
  }
  ...
  async searchMemoriesByEmbedding(
    ...
    const result = await this.client.rpc("search_memories", queryParams);
    if (result.error) {
      throw new Error(JSON.stringify(result.error));
    }
    return result.data.map((memory) => ({
  ...
  async getMemoryById(memoryId: UUID): Promise<Memory | null> {
    try {
      const { data, error } = await this.client.from("memories").select("*").eq("id", memoryId).maybeSingle(); // Use maybeSingle() instead of single()
  ...
  async getMemoriesByIds(memoryIds: UUID[], tableName?: string): Promise<Memory[]> {
    ...
    let query = this.client.from("memories").select("*").in("id", memoryIds);
  ...
  async createMemory(memory: Memory, tableName: string, unique = false): Promise<void> {
    ...
    if (unique) {
      ...
      const result = await this.client.rpc("check_similarity_and_insert", opts);

      if (result.error) {
        throw new Error(JSON.stringify(result.error));
      }
    } else {
      const result = await this.client.from("memories").insert({ ...memory, createdAt, type: tableName });
      const { error } = result;
      if (error) {
  ...
  async removeMemory(memoryId: UUID): Promise<void> {
    const result = await this.client.from("memories").delete().eq("id", memoryId);
    const { error } = result;
    if (error) {
      throw new Error(JSON.stringify(error));
    }
  ...
  async removeAllMemories(roomId: UUID, tableName: string): Promise<void> {
    const result = await this.client.rpc("remove_memories", {
      query_table_name: tableName,
      query_roomid: roomId
    });
  ...
  async countMemories(roomId: UUID, unique = true, tableName: string): Promise<number> {
    if (!tableName) {
    ...
    const result = await this.client.rpc("count_memories", query);
  ...
  async getGoals(params: { roomId: UUID; userId?: UUID | null; onlyInProgress?: boolean; count?: number }): Promise<Goal[]> {
    ...
    const { data: goals, error } = await this.client.rpc("get_goals", opts);
  ...
  async updateGoal(goal: Goal): Promise<void> {
    const { error } = await this.client.from("goals").update(goal).match({ id: goal.id });
    if (error) {
  ...
  async createGoal(goal: Goal): Promise<void> {
    const { error } = await this.client.from("goals").insert(goal);
    if (error) {
      throw new Error(`Error creating goal: ${error.message}`);
    }
  }
  ...
  async removeGoal(goalId: UUID): Promise<void> {
    const { error } = await this.client.from("goals").delete().eq("id", goalId);
    if (error) {
  ...
  async removeAllGoals(roomId: UUID): Promise<void> {
    const { error } = await this.client.from("goals").delete().eq("roomId", roomId);
    if (error) {
  ...
  async getRoomsForParticipant(userId: UUID): Promise<UUID[]> {
    const { data, error } = await this.client.from("participants").select("roomId").eq("userId", userId);
  ...
  async getRoomsForParticipants(userIds: UUID[]): Promise<UUID[]> {
    const { data, error } = await this.client.from("participants").select("roomId").in("userId", userIds);

    if (error) {
      throw new Error(`Error getting rooms by participants: ${error.message}`);
    }

    return Array.from(new Set(data.map((row) => row.roomId as UUID)));
  }
  ...
  async addParticipant(userId: UUID, roomId: UUID): Promise<boolean> {
    const { error } = await this.client.from("participants").insert({ userId: userId, roomId: roomId });
  ...
  async removeParticipant(userId: UUID, roomId: UUID): Promise<boolean> {
    const { error } = await this.client.from("participants").delete().eq("userId", userId).eq("roomId", roomId);
  ...
  async createRelationship(params: { userA: UUID; userB: UUID }): Promise<boolean> {
    const allRoomData = await this.getRoomsForParticipants([params.userA, params.userB]);

    let roomId: UUID;

    if (!allRoomData || allRoomData.length === 0) {
      // If no existing room is found, create a new room
      const { data: newRoomData, error: roomsError } = await this.client.from("rooms").insert({}).single();
    ...
    const { error: participantsError } = await this.client.from("participants").insert([
      { userId: params.userA, roomId },
      { userId: params.userB, roomId }
    ]);

    if (participantsError) {
      throw new Error("Participants creation error: " + participantsError.message);
    }

    // Create or update the relationship between the two users
    const { error: relationshipError } = await this.client
      .from("relationships")
      .upsert({
        userA: params.userA,
        userB: params.userB,
  ...
  async getRelationship(params: { userA: UUID; userB: UUID }): Promise<Relationship | null> {
    const { data, error } = await this.client.rpc("get_relationship", {
      usera: params.userA,
      userb: params.userB
    });
  ...
  async getRelationships(params: { userId: UUID }): Promise<Relationship[]> {
    const { data, error } = await this.client.from("relationships").select("*").or(`userA.eq.${params.userId},userB.eq.${params.userId}`).eq("status", "FRIENDS");
  ...
  async getCache(params: { key: string; agentId: UUID }): Promise<string | undefined> {
    const { data, error } = await this.client.from("cache").select("value").eq("key", params.key).eq("agentId", params.agentId).maybeSingle();
  ...
  async setCache(params: { key: string; agentId: UUID; value: string }): Promise<boolean> {
    const { error } = await this.client.from("cache").upsert({
      key: params.key,
  ...
  async deleteCache(params: { key: string; agentId: UUID }): Promise<boolean> {
    try {
      const { error } = await this.client.from("cache").delete().eq("key", params.key).eq("agentId", params.agentId);
      ...
      elizaLogger.error("Database connection error in deleteCache", error instanceof Error ? error.message : String(error));
      return false;
    }
  }

  async getKnowledge(params: { id?: UUID; agentId: UUID; limit?: number; query?: string }): Promise<RAGKnowledgeItem[]> {
    let query = this.client.from("knowledge").select("*").or(`agentId.eq.${params.agentId},isShared.eq.true`);
  ...
  async searchKnowledge(params: { agentId: UUID; embedding: Float32Array; match_threshold: number; match_count: number; searchText?: string }): Promise<RAGKnowledgeItem[]> {
    ...
    const { data, error } = await this.client.rpc("search_knowledge", {
      query_embedding: embedding,
  ...
  async createKnowledge(knowledge: RAGKnowledgeItem): Promise<void> {
    try {
      ...
      const { error } = await this.client.from("knowledge").insert({
        id: knowledge.id,
  ...
  async removeKnowledge(id: UUID): Promise<void> {
    const { error } = await this.client.from("knowledge").delete().eq("id", id);
  ...
  async clearKnowledge(agentId: UUID, shared?: boolean): Promise<void> {
    if (shared) {
      const { error } = await this.client.from("knowledge").delete().filter("agentId", "eq", agentId).filter("isShared", "eq", true);
      ...
      const { error } = await this.client.from("knowledge").delete().eq("agentId", agentId);
...
export const supabaseAdapter: Adapter = {
  init: (runtime: IAgentRuntime) => {
    ...
    if (supabaseUrl && supabaseAnonKey) {
      ...
      const db = new SupabaseDatabaseAdapter(supabaseUrl, supabaseAnonKey, elizaLogger);
